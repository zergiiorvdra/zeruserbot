...
from Zergio import app, CMD_HELP
from Zergio.helpers.cmd import *
from Zergio.helpers.PyroHelpers import ReplyCheck
from Zergio.helpers.utility import split_list
...
async def module_help(client: Client, message: Message):
    ...
    if len(cmd) > 1:
        ...
        try:
            ...
            ac = PrettyTable()
            ac.header = False
            ac.title = "zergiouserbot Plugins"
            ac.align = "l"
            for x in split_list(sorted(CMD_HELP.keys()), 2):
                ac.add_row([x[0], x[1] if len(x) >= 2 else None])
            xx = await client.send_message(
                message.chat.id,
                f"```{str(ac)}```\n• @MUTUALAN_CARI_TEMAN_VIRTUAL × @ZERGIIORVDRA •",
...
async def module_helper(client: Client, message: Message):
    ...
    if len(cmd) > 1:
        ...
        ac = PrettyTable()
        ac.header = False
        ac.title = "Zergio-UserBot Plugins"
        ac.align = "l"
        for x in split_list(sorted(CMD_HELP.keys()), 2):
            ac.add_row([x[0], x[1] if len(x) >= 2 else None])
        await edit_or_reply(
