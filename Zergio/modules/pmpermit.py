...
async def incomingpm(client: Client, message: Message):
    try:
        from Zergio.helpers.SQL.globals import gvarstatus
        from Zergio.helpers.SQL.pm_permit_sql import is_approved
...
async def auto_accept(client, message):
    try:
        from Zergio.helpers.SQL.pm_permit_sql import approve, is_approved
    except BaseException:
        pass
...
async def approvepm(client: Client, message: Message):
    try:
        from Zergio.helpers.SQL.pm_permit_sql import approve
    except BaseException:
...
async def disapprovepm(client: Client, message: Message):
    try:
        from Zergio.helpers.SQL.pm_permit_sql import dissprove
    except BaseException:
        await message.edit("Running on Non-SQL mode!")
        return
...
async def setpm_limit(client: Client, cust_msg: Message):
    ...
    try:
        from Zergio.helpers.SQL.globals import addgvar
...
async def setpmpermit(client: Client, cust_msg: Message):
    ...
    try:
        import Zergio.helpers.SQL.globals as sql
    except AttributeError:
        await cust_msg.edit("**Running on Non-SQL mode!**")
        return
    Man = await cust_msg.edit("`Processing...`")
