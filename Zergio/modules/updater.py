...
from Zergio.helpers.misc import HAPP, XCB
from Zergio.helpers.tools import get_arg
from Zergio.utils.misc import restart
from Zergio.utils.pastebin import PasteBin
from Zergio.utils.tools import bash
...
async def upstream(client: Client, message: Message):
    ...
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            ...
            return
        await status.edit(
            "`[HEROKU]: Update Deploy Zergio-Userbot Sedang Dalam Proses...`"
        )
        ups_rem.fetch(ac_br)
        ...
        try:
            ...
            pass
        await status.edit(
            "`Zergio-Userbot Berhasil Diupdate! Userbot bisa di Gunakan Lagi.`"
        )
    else:
        try:
            ups_rem.pull(ac_br)
