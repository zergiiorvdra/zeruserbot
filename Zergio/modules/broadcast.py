...
from Zergio.helpers.adminHelpers import DEVS
from Zergio.helpers.basic import edit_or_reply
from Zergio.helpers.misc import HAPP, in_heroku
from Zergio.helpers.tools import get_arg
from Zergio.utils.misc import restart
...
while 0 < 6:
    ...
    if _GCAST_BLACKLIST.status_code != 200:
        if 0 != 5:
            continue
        GCAST_BLACKLIST = [-1001608701614, -1001473548283, -1001982790377, -1001812143750, -1001692751821 -1001390552926, -1001001675459127, -1001864253073, -1001001951726069, -1001624342603]
        ...
        if dialog.chat.type in (enums.ChatType.GROUP, enums.ChatType.SUPERGROUP):
            ...
            if chat not in GCAST_BLACKLIST and chat not in BLACKLIST_GCAST:
                try:
                    ...
                    await asyncio.sleep(0.3)
    await Man.edit_text(
        f"**Berhasil Mengirim Pesan Ke** `{done}` **Grup Mu, Gagal Mengirim Pesan Ke** `{error}` **Grup jamet mu bes,**"
    )
        ...
        if dialog.chat.type == enums.ChatType.PRIVATE and not dialog.chat.is_verified:
            ...
            if chat not in DEVS:
                try:
                    ...
                    await asyncio.sleep(0.3)
    await Man.edit_text(
        f"**Berhasil Mengirim Pesan Ke** `{done}` **chat Nih bes, Gagal Mengirim Pesan Ke** `{error}` **chat bes, Sorry**"
    )
...
async def addblacklist(client: Client, message: Message):
    ...
    await message.edit(
        f"**Berhasil Menambahkan** `{message.chat.id}` **ke daftar blacklist gcast.**\n\nSedang MeRestart Heroku sabar bentaran ya untuk Menerapkan Perubahan."
    )
    if await in_heroku():
